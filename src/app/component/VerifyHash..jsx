...
export default function VerifyHash({ setActiveModal }) {
    ...
    return (
        <div className="w-full flex flex-col items-start justify-center mt-16 px-8">
            ...
            {hash && (
                <div className="w-full max-w-2xl mb-6 bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 rounded-lg p-4 shadow-lg">
                    <div className="flex items-center gap-2 mb-2">
                        <Hash className="w-4 h-4 text-blue-400" />
                        <p className="text-gray-400 text-sm font-medium">SHA256 Hash:</p>
                    </div>
                    <p className="text-gray-100 font-mono text-xs leading-relaxed break-all bg-black/30 p-3 rounded border border-gray-800">{hash}</p>
                </div>
            )}
      ...
      ${hash
            ...
            {verified === true && (
                <div className="border border-green-500/30 bg-gradient-to-br from-green-950/40 to-gray-900/40 backdrop-blur-sm rounded-xl p-6 mt-8 w-full max-w-2xl shadow-xl">
                    <div className="flex items-start gap-3 mb-4">
                        <div className="p-2 bg-green-500/10 rounded-lg">
                            <CircleCheckBig className="w-6 h-6 text-green-400" />
                        </div>
                        <div>
                            <h3 className="text-green-400 font-semibold text-lg mb-1">Verification Successful</h3>
                            <p className="text-gray-400 text-sm leading-relaxed">The file's SHA-256 hash matches the record stored on Solana, confirming it hasn't been modified since upload.</p>
                        </div>
                    </div>

                    {/* display file details directly (fields come from DB) */}
                    {file && (
                        <div className="mt-4 bg-gradient-to-br from-gray-900 to-black border border-gray-800 rounded-lg p-5 shadow-inner">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1 space-y-3">
                                    <div>
                                        <p className="text-base font-semibold text-gray-100 mb-1">{file?.[0]?.file_name}</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 gap-3">
                                        <div className="flex items-center gap-2 text-xs">
                                            ...
                                            <span className="text-gray-300 font-mono bg-gray-800/50 px-2 py-1 rounded">{file?.[0]?.wallet_address}</span>
                                        </div>

                                        <div className="flex items-center gap-2 text-xs">
                                            <Calendar className="w-4 h-4 text-gray-500" />
                                            <span className="text-gray-500 font-medium">Timestamp:</span>
                                            <span className="text-gray-300">{new Date(file?.[0]?.created_at).toLocaleString()}</span>
                                        </div>

                                        <div className="flex items-start gap-2 text-xs">
                                            <ExternalLink className="w-4 h-4 text-gray-500 mt-0.5" />
                                            <span className="text-gray-500 font-medium">Transaction:</span>
                                            <span className="text-gray-300 font-mono break-all bg-gray-800/50 px-2 py-1 rounded flex-1">{file?.[0]?.transaction_signature}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            {verified === false && (
                <div className="mt-6 w-full max-w-2xl bg-gradient-to-br from-red-950/40 to-gray-900/40 border border-red-500/30 rounded-lg p-4 flex items-center gap-3">
                    <div className="text-red-400 text-xl">❌</div>
                    <p className="text-red-400 font-medium">Hash not found on-chain. This file may not be registered or has been altered.</p>
                </div>
            )} 
            {infoTooltip && (
                <div className="absolute top-7   right-20 w-80 bg-gray-800 border border-gray-700 rounded-lg p-4 shadow-lg z-50">
                    <div className="flex justify-between items-center mb-2">
                        <h4 className="text-white font-semibold">What is Hash Verification?</h4>
                        <button onClick={() => setInfoTooltip("")} className="text-gray-400 hover:text-gray-200">&times;</button>
                    </div>
