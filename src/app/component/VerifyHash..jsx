...
import { FileText, CircleCheckBig, Shield, Calendar, User, Hash, ExternalLink, Info } from 'lucide-react';
...
export default function VerifyHash({ setActiveModal }) {
        ...
        try {
            ...
            reader.onload = () => {
                try {
                    ...
                    console.error("Error computing file hash:", hashErr);
    ...
    async function verifyHashOnChain(hash, walletAddress) {
        try {
            ...
            if (!walletAddress || !connected) {
                setError("Please connect your wallet before verifying the hash.");
                setLoading(false);
                return false;
            }
            ...
            setError("An error occurred while verifying the hash on Solana.");
            setVerified(false);
            setLoading(false);
            return false;
        }
    ...
    const fetchFile = async (hash) => {
        try {
            ...
            if (!hash) {
                setError("Missing hash for file lookup.");
            ...
            setError("Failed to fetch file details from the server.");
        } finally {
            setLoadingFile(false);
        }
    };
    ...
    return (
        <div className="w-full flex flex-col items-start justify-center mt-16 px-8">
            ...
            {loading ? <LoaderAnimation /> : (
                <button
                    ...
                    className={`w-full max-w-2xl py-3 rounded-lg font-semibold text-center transition-all duration-200 shadow-lg
                        ${hash
                            ? "bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 dark:from-blue-600 dark:to-blue-700 dark:hover:from-blue-700 dark:hover:to-blue-800 text-white transform hover:scale-[1.01]"
            ...
            {error && (
                <div className="mt-4 w-full max-w-2xl bg-red-100/60 dark:bg-red-950/40 border border-red-400/50 dark:border-red-500/30 rounded-lg p-3 text-red-700 dark:text-red-400 text-sm font-medium shadow">
                    {error}
                </div>
            )}
